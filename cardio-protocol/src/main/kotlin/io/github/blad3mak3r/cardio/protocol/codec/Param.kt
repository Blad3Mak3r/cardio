...
fun Any?.toParam(): Param<*> = when (this) {
    ...
    is List<*> -> {
        ...
        when {
            ...
            first is Int            -> Param(this as List<Int?>,               Int4ArrayCodec)
            first is Short          -> Param(this as List<Short?>,             Int2ArrayCodec)
            first is Long           -> Param(this as List<Long?>,              Int8ArrayCodec)
            first is Float          -> Param(this as List<Float?>,             Float4ArrayCodec)
            first is Double         -> Param(this as List<Double?>,            Float8ArrayCodec)
            first is String         -> Param(this as List<String?>,            TextArrayCodec)
            first is Boolean        -> Param(this as List<Boolean?>,           BoolArrayCodec)
            first is kotlin.uuid.Uuid -> Param(this as List<kotlin.uuid.Uuid?>, KotlinUuidArrayCodec)
            first is kotlin.time.Instant -> Param(this as List<kotlin.time.Instant?>, TimestamptzArrayCodec)
            else -> error(
